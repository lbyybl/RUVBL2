...
getgeneofpeak <- function(df){
  gene <- lapply(df$name,function(x){
    x <- stringr::str_split(x,'_')[[1]][3]
    return(x)
  }) %>% unlist()
  gene_convid <- clusterProfiler::bitr(gene,fromType = 'SYMBOL',toType = c('ENTREZID'),
                                       OrgDb = org.Mm.eg.db)
  peakAnno <- as.data.frame(gene_convid)
  names(peakAnno) <- c('gene','geneId')
  return(peakAnno)
}
...
pair.gene.fold <- function(data_peak){
  merge_data <- data_peak %>%
    dplyr::select(name,Fold)
  merge_data$name <- lapply(merge_data$name,function(x){
    x <- stringr::str_split(x,'_')[[1]][3]
    return(x)
  }) %>% unlist()
  names(merge_data) <- c('SYMBOL','Fold')
  data.out <- merge_data
  data.out <- data.out %>% 
...
if (nrow(up)>0){
  ...
  up.gene.pair <- pair.gene.fold(up)
...
if (nrow(down)>0){
  ...
  down.gene.pair <- pair.gene.fold(down)
