...
dbObj <- dba.count(dbObj, bUseSummarizeOverlaps=TRUE, summits=FALSE,filter = 0,minCount = 0, mapQCth = 0, bParallel = TRUE,
                   score = 'DBA_SCORE_NORMALIZED')
#--- normalize
dbObj <- dba.normalize(dbObj,normalize = DBA_NORM_RLE,spikein = T)
...
for (i in 1:length(dbObj$contrasts)){
  ...
  dbObj.DB <- dba.report(dbObj, method=DBA_DESEQ2, contrast = i, th=1,bFlip=F)
  ...
  if (! usepvalue){
    ...
    if (xrange != FALSE & yrange != FALSE){
      if (',' %in% paste0(xrange)) {xrange <- strsplit(xrange, split = ",")[[1]] %>% as.numeric()}
      if (',' %in% paste0(yrange)) {yrange <- strsplit(yrange, split = ",")[[1]] %>% as.numeric()}
      dba.plotMA(dbObj, method = DBA_DESEQ2, contrast = i,bNormalized = T, 
                 bXY = F, bSmooth = T,  th = fdrtr, fold = fctr, bLoess = F, 
                 xrange = xrange, yrange = yrange,bFlip = F)
    ...
    if (xrange != FALSE & yrange != FALSE){
      ...
      dba.plotMA(dbObj, method = DBA_DESEQ2, contrast = i, bXY = F, 
                 ...
                 bLoess = F, xrange = xrange, yrange = yrange,bFlip = F)
    }else if (xrange != FALSE & yrange == FALSE){
      if (',' %in% paste0(xrange)) {xrange <- strsplit(xrange, split = ",")[[1]] %>% as.numeric()}
	  yrange <- c(-max_fold,max_fold)
      dba.plotMA(dbObj, method = DBA_DESEQ2, contrast = i, bXY = F, 
      ...
      dba.plotMA(dbObj, method = DBA_DESEQ2, contrast = i, bXY = F, 
                 bSmooth = T, bUsePval = T, th = fdrtr, fold = fctr, 
                 bLoess = F, yrange = yrange,bFlip = F)
    }else{
	  yrange <- c(-max_fold,max_fold)
      dba.plotMA(dbObj, method = DBA_DESEQ2, contrast = i, bXY = F, 
                 bSmooth = T, bUsePval = T, th = fdrtr, fold = fctr, 
