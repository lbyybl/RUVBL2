#--- this script is used to draw the meta of pro-seq Genebody
...
step <- 1100
data <- fread('gene_region.gz',stringsAsFactors = F,skip = 1)
...
data_sts$coor <- rep(seq(1,1100,length.out = step),2*sample_num)
...
ggplot(data_sts,aes(coor,value,fill=type,color=sample))+geom_line()+
  scale_x_continuous(limits=c(1,1100),breaks = c(1,300,800,1100),
                     labels = c('-3000','TSS','TES','3000'))+xlab('')+
